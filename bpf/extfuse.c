...
/*
        ...
        BPF_MAP_TYPE_LRU_PERCPU_HASH: all cores share one hash-table but have
   they own LRU structures of the table.
*/
...
{
	int64_t ret =
		bpf_fuse_read_args(ctx, NODEID, &key->nodeid, sizeof(u64));
	if (ret < 0) {
...
HANDLER(FUSE_GETATTR)(void *ctx)
{
	...
	if (attr->stale) {
		...
		ret = bpf_fuse_read_args(ctx, IN_PARAM_0_VALUE, &inarg,
					 sizeof(inarg));
		if (ret < 0) {
	...
	/* populate output */
	ret = bpf_fuse_write_args(ctx, OUT_PARAM_0, &attr->out,
				  sizeof(attr->out));
	if (ret) {
...
#endif
	/* mark as stale to prevent future references to cached attrs */
	__sync_fetch_and_add(&attr->stale,
